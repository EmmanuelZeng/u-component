...
const Popper = ({container, children, togglerClass, position, className}) => {
    return ReactDOM.createPortal(
        <div id="dropdown-states" className={`${togglerClass} w-fit z-10 rounded-lg shadow-lg border-gray1 absolute top-[3rem] ${className} ${position}`}>
            {children}
        </div>,
        container
    )
...
const ClickAway = ({
    ...
    return (
        <div ref={containerRef} className={`${containerClassName} ${width} relative h-10`}>
            {TriggerComponent && <TriggerComponent {...otherTriggerProps} onClick={toggleOptions} />}
            {!TriggerComponent &&
                <Button
                    {...otherTriggerProps}
                    id="states-button"
            ...
            {open &&
                <Popper className={popoverClassName} position={pos} container={portalContainer || containerRef.current} togglerClass={togglerClass}>
...
ClickAway.propTypes = {
     ...
     */
    triggerType: PropTypes.oneOf(["default", "primary", "info", "danger", "success"]),
    /**
     * The variant to use.
     */
